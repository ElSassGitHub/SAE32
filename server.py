...
def register_msg(msg, login, salon):
    """
    Fonction enregistrant un message envoyé par un client dans la base de données
    
    :param msg: message envoyé par un client connecté au serveur
    :type msg: str

    :param login: identifiant de connection de l'émetteur du message
    :type login: str
    
    :param salon: salon depuis lequel le message a été envoyé
    :type salon: str
    """
...
def check_database():
    """
    Fonction récupérant les informations de la base de données et qui les insère dans des variables pythons pour leur référencement dans le code
    """
...
def connection(server_socket):
    """
    Fonction initialisant la connection à la base de données et les variables accueillant ses informations et qui met le serveur à l'écoute pour de nouvelles collections

    :param server_socket: objet contenant les détail de la connection offerte par le serveur
    """
...
def receive(conn, addr):
    """
    Fonction connectant l'utilisateur à un compte (si non ban), puis gérant les messages reçus par ce client

    :param conn: objet contenant les détails de la connection avec le client
    :param addr: contient l'adresse IP du client ainsi que le port qu'il utilise pour sa connection
    :type addr: tuple
    """
    ...
    while flag != False and flag_all != False:
        ...
        if msg == "/bye":
            ...
            if command[1].startswith("@"):
                ...
                print(target_pseudo)
                print(aliases)
                if target_pseudo in aliases.values():
...
def main():
    """
    Fonction initialisant le serveur
    """

    host = input("Entrez la plage d'adresses acceptables [0.0.0.0]: ")
    if host == "":
        host = '0.0.0.0'
    port = input("Entrez le port d'écoute [10000]: ")
    if port == "":
        port = 10000
    else:
        port = int(port)
    ...
    wait_co = threading.Thread(target=connection, args=[server_socket])
    wait_co.start()
